...
```{r}
dt_KPI_raw[dt_KPI_raw$Premium<0 ,]
#leave out neg. Premium
dt_KPI_raw.PP <- dt_KPI_raw[dt_KPI_raw$Premium>=0,]
summary(dt_KPI_raw.PP)
#similarly with negative Losses and Expenses (NonNegative dataset)
dt_KPI_raw.NN <- dt_KPI_raw[(dt_KPI_raw$Premium>=0) & (dt_KPI_raw$Losses>=0) & (dt_KPI_raw$Expenses>=0) ,]
summary(dt_KPI_raw.NN)
```
...
dt_KPI_raw[dt_KPI_raw$Unit=="Unit5",] %>%
  group_by(Business) %>%
  summarize(Expenses = sum(Expenses, na.rm=TRUE)) %>%
  arrange(desc(Expenses))
...
##### Which Business from Unit found in the previous task had the highest Expenses? 
```{r}
#dt_KPI_raw2 <- dt_KPI_raw[na.omit(dt_KPI_raw$Business),]
dt_KPI_raw[dt_KPI_raw$Unit=="Unit5",] %>% 
  group_by(Business) %>% 
  summarize(Premium = sum(Premium, na.rm = TRUE),
            Expenses = sum(Expenses, na.rm = TRUE),
...
dt_KPI_raw %>%
  ...
  summarize (Expenses = sum(Expenses, na.rm=TRUE),
             Losses = sum(Losses, na.rm=TRUE)) %>%
  ggplot(aes(x = reorder(Unit, Losses), y = Expenses)) + 
  geom_col()
```
...
dt_KPI_raw %>%
  mutate(uwr=Premium-Expenses-Losses) %>% 
  group_by(Unit) %>%
  summarise(uwr= sum(uwr, na.rm=TRUE))  %>%
  arrange(desc(uwr))
```
...
```{r}
# your code
dt_KPI_raw[dt_KPI_raw$Unit=="Unit7",] %>%
  mutate(uwr=Premium-Expenses-Losses) %>% 
  group_by(Business) %>%
  summarise(uwr= sum(uwr, na.rm=TRUE))  %>%
  arrange(desc(uwr))
...
###HW2### Find out, which __year__ was the __worst__ for portfolio you have identified as __most profitable__ during the lesson and show it on the chart using `ggplot2` package. Write an explanation about your findings into the code as comment. __Commit__ it to your repository into `Lessons/Lesson2/Homework`.
--------------------------------------------------------------------------------
```{r}
dt_KPI_raw[dt_KPI_raw$Unit=="Unit7",] %>%
  mutate(uwr=Premium-Expenses-Losses) %>% 
  ...
  geom_col()
####the worst year was 2014, because in that year was the underwriting result at it's lowest.
```
